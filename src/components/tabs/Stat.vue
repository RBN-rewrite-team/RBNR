...
<template>
  ...
  font-size: 500px;
  line-height: 300px;
  transform: translateX(-50%);">ω</span><br>
  
      <div>
			  当前序数等级：{{getOrdinalLevel()}}<br>
			  下一序数等级要求：<vue-latex :expression="OrdinalUtils.numberToLaTeXOrdinal(new Ordinal(ordinalNormal[getOrdinalLevel()]).toDecimal(feature.Ordinal.base()), feature.Ordinal.base().toNumber()) ?? '\\textit{way too large}'" />
			</div>
		</div>
	</div>
</template>
