	...
	<div class="main">
		<div style="display: flex; justify-content: center; align-items: center; margin-top: 5px">
			<button
				...
				@click="player.help.page = Math.min(Math.max(player.help.page - 1, 1), 1000)"
			>
				-
			</button>
			...
			<button class="clickable_button" @click="player.help.page = 777" v-if="isDeveloper()">
				开发者模式
			</button>
		</div>
		<div align="center">
		...
		<div v-if="player.help.page == 1">
			序数是数学中用来表示“顺序”的数，它扩展了我们熟悉的自然数(0, 1, 2, 3
			...)，可以表示无限序列中的位置。<br />
			序数具有以下规则：<br />
			1. 一个数的下一个数仍然是一个序数<br />
			2. 对于任意由序数构成的集合，存在一个最小的序数，它大于这集合中所有的序数。<br /><br />
			上述两条规则给出了两种不同的序数，我们分别称之为后继序数和极限序数，它们的具体定义如下：<br />
			1. 对任意序数<vue-latex expression="\alpha" />，存在一个序数<vue-latex
				expression="\alpha'=\alpha+1"
			/>，我们称之为后继序数。<br />
			2. 对于任意由序数构成的序数数列<vue-latex
				expression="\{\alpha_n\}"
			/>，存在一个序数<vue-latex expression="\beta" />满足<vue-latex
				expression="\beta = \sup_n\{\alpha_n\}"
			/><br />
			这里，<vue-latex
				expression="\sup"
			/>为上确界，它指的就是大于（或等于）集合中所有元素的最小元素。<br />
			我们称<vue-latex expression="\beta" />为极限序数，<vue-latex
				expression="\{\alpha_n\} = \{\alpha_0, \alpha_1, \alpha_2, \cdots\}"
			/>称为<vue-latex expression="\beta" />的基本列，记作<vue-latex
				expression="\beta[n] = a_n"
			/><br /><br />
			需要注意的是，基本列的指标默认是从 0 开始的，也就是说一个极限序数<vue-latex
				expression="\beta"
			/>应当表示为
			<vue-latex
				expression="\beta = \sup_n\{\beta[n]\} = \sup\{\beta[0], \beta[1], \beta[2], \cdots\}"
				display-mode
			/>
		...
		<div v-if="player.help.page == 2">
			...
			序数的加法定义为：<br />
			1. 对任意序数<vue-latex expression="\alpha" />，<vue-latex
				expression="\alpha+0 = \alpha"
			/><br />
			2. 对任意序数<vue-latex expression="\alpha, \beta" />，<vue-latex
				expression="\alpha+\beta' = (\alpha+\beta)'"
			/><br />
			3. 对任意序数<vue-latex expression="\alpha" />和极限序数<vue-latex
				expression="\beta"
			/>，<vue-latex expression="\alpha+\beta = \sup\{\alpha+γ|γ<\beta\}" /><br />
			特别地，如果我们考虑的极限序数的基本列为<vue-latex
				expression="\beta[n]"
			/>，那么上述第三条规则可以进一步表示为
			...
			类似的，序数的乘法定义为：<br />
			1. 对任意序数<vue-latex expression="\alpha" />，<vue-latex
				expression="\alpha\cdot0 = 0"
			/><br />
			2. 对任意序数<vue-latex expression="\alpha, \beta" />，<vue-latex
				expression="\alpha\cdot\beta' = \alpha\cdot\beta+\alpha"
			/><br />
			3. 对任意序数<vue-latex expression="\alpha" />和极限序数<vue-latex
				expression="\beta"
			/>，<vue-latex expression="\alpha\cdot\beta = \sup\{\alpha\cdotγ|γ<\beta\}" /><br />
			特别地，如果我们考虑的极限序数的基本列为<vue-latex
				expression="\beta[n]"
			/>，那么上述第三条规则可以进一步表示为
			...
			<vue-latex
				expression="(\alpha\cdot\beta)\cdotγ = \alpha\cdot(\beta\cdotγ),"
				display-mode
			/>
			<vue-latex
				expression="\alpha\cdot(\beta+γ) = \alpha\cdot\beta+\alpha\cdotγ"
				display-mode
			/>
			右分配律是不满足的，例如：
			<vue-latex
				expression="(\omega+1)\cdot2 = (\omega+1)+(\omega+1) = \omega+(1+\omega)+1 = \omega+\omega+1 = \omega\cdot2+1"
				display-mode
			/>
			...
			/><br />
			2. 对任意序数<vue-latex expression="\alpha, \beta" />，<vue-latex
				expression="\alpha^{\beta'} = \alpha^\beta\cdot\alpha"
			/><br />
			3. 对任意序数<vue-latex expression="\alpha" />和极限序数<vue-latex
				expression="\beta"
			/>，<vue-latex expression="\alpha^\beta = \sup\{\alpha^γ|γ<\beta\}" /><br />
			...
			我们称某个序数<vue-latex expression="\beta" />是<vue-latex
				expression="\alpha\mapsto f(\alpha)"
			/>的不动点，指的是若将<vue-latex expression="\beta" />变换为<vue-latex
				expression="f(\beta)"
			/>，有<vue-latex expression="\beta = f(\beta)" />
		</div>
		...
		<div v-if="player.help.page == 4 && player.upgrades[58]">
			...
			那就是利用不同基本列所定义的极限序数所对应的函数<vue-latex
				expression="f_\alpha(n) = f_{a[n]}(n)"
			/>是不同的。<br />
			...
			上述两个基本列可以给出相同的极限序数<vue-latex
				expression="\omega"
			/>，但是它们所对应的函数的增长速度却并不相同。<br />
			为此，我们需要定义“标准基本列”，以便对这种情况进行规范。<br />
			但在此之前，由于形如<vue-latex
				expression="1+\omega=\omega"
			/>这种结果，在讨论序数的基本列之前，我们需要先对序数进行标准化。<br />
			我们通常使用的标准形式是 Cantor 标准型，它定义为如下形式：<br />
			<vue-latex
				expression="\alpha = \omega^{\alpha_0}+\omega^{\alpha_1}+\cdots\omega^{\alpha_n},"
				display-mode
			/>
			其中，<vue-latex
				expression="\alpha_0\ge a_1\ge \cdots\ge a_n"
			/>。可以证明，任意一个序数都可以写成 Cantor 标准型。<br />
			例如，对于序数
			<vue-latex
				expression="\omega^{\omega^{\omega^{19728}}+\omega^{\omega}\cdot3+2}\cdot2+\omega^{\omega^3+1}+1"
				display-mode
			/>
			我们可以序数乘法展开，并稍微变形，得到
			<vue-latex
				display-mode
				expression="\omega^{\omega^{\omega^{19728}}+\omega^{\omega^1}+\omega^{\omega^1}+\omega^{\omega^1}+\omega^0+\omega^0}+\omega^{\omega^{\omega^{19728}}+\omega^{\omega^1}+\omega^{\omega^1}+\omega^{\omega^1}+\omega^0+\omega^0}+\omega^{\omega^3+\omega^0}+\omega^0"
			/>
			我们可以递归地对<vue-latex
				expression="\varepsilon_0"
			/>之前的序数定义如下标准形式：<br />
			1. 0是标准形式<br />
			2. 如果<vue-latex expression="\alpha, \beta" />是标准形式<br />而且<vue-latex
				expression="\alpha \ge \beta"
			/>，那么<vue-latex expression="\alpha+\beta" />是标准形式<br />
			3. 如果<vue-latex expression="\alpha" />是标准形式<br />而且<vue-latex
				expression="\omega^\alpha>\alpha"
			/>，那么<vue-latex expression="\omega^\alpha" />是标准形式<br />
			容易验证，所有的自然数<vue-latex
				expression="n"
			/>都是标准形式。值得注意的是，上述定义中并没有涉及序数乘法。<br />
			习惯上，我们仍然容许<vue-latex
				expression="\alpha\cdot n"
			/>这样的表达式存在，作为<vue-latex
				expression="\underbrace{\alpha+\alpha+\cdots+\alpha}_{n\text{个}\alpha}"
			/>的简写。<br />
			由第二条规则可知，若<vue-latex expression="\alpha" />是标准形式，n是自然数，则<vue-latex
				expression="\alpha\cdot n"
			/>也是标准形式<br />
			在转换为标准形式后，我们便可以定义极限序数的标准基本列了<br />
			<vue-latex expression="\varepsilon_0" />以下极限序数的标准基本列定义为：<br />
			1. <vue-latex expression="\omega[n] = n" /><br />
			2. 对于任意序数<vue-latex expression="\alpha" />，极限序数<vue-latex
				expression="\beta"
			/>，自然数<vue-latex expression="n" />，<vue-latex
				expression="(\alpha+\beta)[n] = \alpha+\beta[n]"
			/><br />
			3. 对于任意序数<vue-latex expression="\alpha" />，自然数<vue-latex
				expression="n"
			/>，<vue-latex expression="\omega^{\alpha+1}[n] = \omega^\alpha\cdot n" /><br />
			4. 对于任意极限序数<vue-latex expression="\alpha" />，自然数<vue-latex
				expression="n"
			/>，<vue-latex expression="\omega^\alpha[n] = \omega^{\alpha[n]}" /><br /><br />
			除此之外，根据上述第二条规则，容易得出<vue-latex
				expression="\alpha\cdot(m+1)[n] = \alpha\cdot m+\alpha[n]"
			/>
		</div>
		...
		<div v-else-if="player.help.epsilon && player.help.page == 7">
			...
			4. <vue-latex expression="\varepsilon_{\omega^{\omega^{\varepsilon_0[2]}}}"></vue-latex
			><br />
			5.
			<vue-latex
