      ...
      <div class="background">
        <div class="main" v-if="player.currentTab === 0">
          <div class="clickable">
            ...
            <div class="clickable_button" @mousedown="feature.SUCCESSOR.success()" v-if="!player.upgrades['25']">
			...
			<div class="clickable_button" @mousedown="feature.SUCCESSOR.success()" v-else>
				加法+{{formatWhole(feature.SUCCESSOR.successorBulk())}}<span v-if="BUYABLES.lock('11').unlocked">(自动{{ formatWhole(feature.SUCCESSOR.autoSuccessPerSecond()) }}/s)</span>
			</div>
          </div>
          <table align="center">
		...
		<div class="main" v-if="player.currentTab == 4">
			...
			<div style="transform: translateY(60px)">
				<div align="center" v-if="player.firstResetBit & 0b10">累计乘法能量提供了<span style="color: #cc33ff; font-weight: bold">x{{feature.MULTIPLICATION.powerEff()}}</span>数值增益</div>
				<table align="center">
				  <tbody>
					<tr>
					  <td v-if="UPGRADES.lock('31').show">
						<div
						  class="upgrade"
						  @mousedown="UPGRADES.buy('31')"
						  v-html="UPGRADES.singleHTML('31')"
						></div>
					  </td>
					</tr>
				  </tbody>
		...
		<div class="main" v-if="player.currentTab == 5">
			...
			<div style="transform: translateY(60px)">
				<div align="center">你有<span style="color: #cc33ff; font-weight: bold">x{{formatWhole(feature.PrimeFactor.power())}}</span>因数能量
				<br>基于本次乘法重置耗时提供<span style="color: #cc33ff; font-weight: bold">x{{format(feature.PrimeFactor.powerEff())}}</span>数值和加法能量增益</div>
				<table align="center">
				  <tbody>
					<tr>
