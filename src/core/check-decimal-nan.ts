	...
	const proxy = new Proxy(reactiveTarget, {
		get(target, key, receiver) {
			...
			if (value !== null && typeof value === 'object') {
				return createValidatedReactiveProxy(
					value,
			...
			if (descriptor.value !== undefined) {
				...
				if (value instanceof Decimal) {
					if (isInvalid(value)) {
						if (!(allowNegativePath.includes(currentPath) && !isStrictInvalid(value))) {
							addNotify(
								`Invalid Decimal in defineProperty at path: ${currentPath}, value: ${value.toString()}`,
							);
							console.error(
								`Invalid Decimal in defineProperty at path: ${currentPath}`,
							);
							return false;
						}
					}
