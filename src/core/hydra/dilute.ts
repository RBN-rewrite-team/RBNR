...
const Dil = {
	...
	initMechanics() {
		...
		MILESTONES.create('dut5', {
			...
			/**
			 * VeryrrDefine注：如果推不到2,201,250推这个里程碑 配置为x0x00?100，?开始为0.5,然后随着效果增加，
			 * 让?一直增加，差不多vi到4的时候就推不动了，此时就可以去推2201250了
			 * 然后推到2215313，推不动，然后继续去推这个里程碑，进行一些超越轮回重置
			 * 差不多获得MD13后换成x0000?100继续推这个里程碑，
			 * 接下来用X0000[4.25]100推这个里程碑推到^1.6，
			 * 再一次去推溶液，这时候再推这个里程碑
			 *
			 * 不清楚，应该有朊病毒加成
			 *
		...
		MILESTONES.create('dut8', {
			...
			get canDone() {
				return player.hydra.milestoneDut5Eff.gte(1.25);
			},
			show: true,
			currency: '',
		});
		...
		MILESTONES.create('dut12', {
			...
			get canDone() {
				return player.hydra.milestoneDut5Eff.gte(this.requirement);
			},
			show: true,
			currency: '',
		});
		MILESTONES.create('dut13', {
			...
			get canDone() {
				return player.hydra.power.gte(this.requirement);
			},
			show: true,
			currency: '',
		});
	...
	diluteLoop(diff: number, trueDiff: number) {
		...
		if (player.hydra.dilute.inDilute) {
			...
			if (
				...
				player.hydra.dilute.spentTime > s3Eff.toNumber()
			) {
				ModalService.show({
					title: '已退出稀释',
					content:
	...
	solutionGain(getCurrentMax = false, gettingOutSide = false): Decimal {
		...
		if (gettingOutSide) getAmountFunction = (x: IntClosedRange<0, 8>) => ([10,10,10,10,10,10,true,true,true] as const)[x] as (typeof x extends (0 | 1 | 2 | 3 | 4 | 5) ? 10 : typeof x extends (6|7|8) ? true:never)
