...
export const PrimeFactor = {
	initMechanics() {
        ...
        currency: "乘法能量",
				...
				show() {
					return (this.pfid ?? 1) <= 3 || player.buyables['pf' + this.pprev as keyof typeof player.buyables].gte(1);
				},
			});
		}
	},
	power(){
		...
		for(let i in pflist) base = base.mul(buyables['pf' + pflist[i]].effect(player.buyables['pf' + pflist[i] as keyof typeof player.buyables]));
		return base;
	},
	powerEff(){
