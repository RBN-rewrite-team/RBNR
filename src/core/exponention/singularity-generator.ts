...
export function singularityExponent() {
	const t = player.singularity.t / 1000;
...
export function getSingularityEnergy() {
	const t = player.singularity.t;
	if (t >= converage_point()) return Infinity;
	const a = singularityExponent();
	const b = singularityDivision();
...
export function getSingularityEffect() {
	...
	return new Decimal(2.5).tetrate(getSingularityEnergy()).clampMax('f1e250');
}
