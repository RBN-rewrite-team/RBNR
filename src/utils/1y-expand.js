	...
	while (true) {
		...
		for (var i = 0; i < lastLayer.length; i++) {
			...
			if (calculatedMountain.length == 1) {
				...
				p = 0;
				while (
					calculatedMountain[calculatedMountain.length - 2][p].position <
					lastLayer[i].position + 1
				)
					p++;
			}
			while (true) {
		...
		for (var j = mountain.length - 1; j >= 0; j--) {
			...
			while (true) {
				if (height == 0) {
					...
					while (
						mountain[height - 1][l].position !=
						mountain[height][lastIndex].position + 1
					)
						l++;
				...
				if (!mountain[height][lastIndex] || mountain[height][lastIndex].parentIndex == -1) {
					...
					diagonalTree.push(
						(mountain[height][lastIndex] ? mountain[height][lastIndex].position : -1) +
							height,
					);
					break;
				}
...
function cloneMountain(mountain) {
	if (mountain[0].length === undefined) return calcMountain(mountain.join(','));
	var newMountain = [];
	...
	if (diagonal[0][diagonal[0].length - 1].value != 1) {
		...
		for (var i = mountain.length - 1; i >= 0; i--) {
			if (mountain[i][mountain[i].length - 1].position + i == mountain[0].length - 1)
				return (
					mountain[i - 1][mountain[i - 1][mountain[i - 1].length - 1].parentIndex]
						.position +
					i -
					1
				);
...
export function expand(s, n, stringify) {
	var mountain;
	if (typeof s == 'string') mountain = calcMountain(s);
	else mountain = cloneMountain(s);
		...
		for (var i = 1; i <= n; i++) {
			...
			for (var j = badRootSeam; j < afterCutLength; j++) {
				...
				if (isAscending) {
					for (var k = 0; k < seamHeight + (cutHeight - badRootHeight) * i; k++) {
						...
						if (k < badRootHeight) {
							...
							var parentPosition = mountain[sy][sourceParentIndex]
								? mountain[sy][sourceParentIndex].position +
									parentShifts *
										...
										(mountain[sy][sourceParentIndex].position + sy >=
											badRootSeam) -
									(k - sy)
								: -1;
							var parentIndex = 0;
							...
							result[k].push({
								value:
									parentIndex == -1
										? newDiagonal[0][j + (afterCutLength - badRootSeam) * i]
												.value
										: NaN,
							...
							k <=
							badRootHeight + (cutHeight - badRootHeight) * (i - isReplacingCut)
						) {
							//Br replace
					...
					if (isReplacingCut)
						console.warn('Cut child and not connected to bad root. Makes sense.');
	...
	if (stringify == 2) {
		...
		for (var i = 0; result[0] && i < result[0].length; i++) {
			rr.push(result[0][i].value);
		}
	} else if (stringify) {
		rr = [];
		for (var i = 0; result[0] && i < result[0].length; i++) {
			rr.push(
				result[0][i].value + (result[0].forcedParent ? 'v' + result[0].parentIndex : ''),
			);
		}
		rr = rr.join(',');
	} else {
