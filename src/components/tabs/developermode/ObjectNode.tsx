		...
		return (
			<div class="tree-node">
				<div class="tree-node-header" style={{ paddingLeft }}>
					{hasChildren && (
						<span class="toggle-icon" onClick={this.toggle}>
							{this.isExpanded ? '▼' : '►'}
						</span>
					)}
					...
					<button class="set-value-btn" onClick={() => setVal(this.$parent, keyName)}>
						设置
					</button>
				</div>
				...
				{isObject && this.isExpanded && (
					<div class="tree-node-children">
						{Object.entries(value).map(([key, val]) => (
							<TreeNode value={val} keyName={key} depth={depth + 1} key={key} />
						))}
					</div>
				)}
			</div>
