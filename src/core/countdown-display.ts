...
export function countdown(
	...
	speed: Decimal,
	isconstant = false,
	speed_deri: Decimal = new Decimal(0),
) {
	if (speed_deri.eq(0)) return (isconstant ? '' : `≤`) + formatTime(cost.sub(curres).div(speed));
	else
		return (
			'~' +
			formatTime(
 ...
 */
export function countdown_with_double_derivative(
	cost: Decimal,
	curres: Decimal,
	cur_derivative: Decimal,
	cur_dbl_derivative: Decimal,
) {
