...
function getYSequenceWithoutColon(Y: string): {
  ...
  let type = "???"
  if (String.prototype.startsWith("Y(")) type = "1-Y", Y = Y.slice(2, -1)
  if (String.prototype.startsWith("ω-Y(")) type = "ω-Y", Y = Y.slice(4, -1)
  return {
    type,
    Y
  }
...
function calcDifference(m: NodeMountain): NodeMountain {
  ...
  for (let i = 0; i < rightLegs.length; i++) {
    let pi = i;
    while (pi > -1 && !((rightLegs[pi] as LeafMountain).value < (rightLegs[i] as LeafMountain).value && 
