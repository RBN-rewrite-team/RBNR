...
export default {
  ...
  run(fun: (iteration: number) => void, maxIter: number, config: AsyncConfig): Promise<void> | void {
    if (this.enabled) {
      this.enabled = false
  ...
  async _run(fun: (iteration: number) => void, maxIter: number, config: AsyncConfig): Promise<void> {
    if (!config.progress) config.progress = {}

    config.progress.maxIter = maxIter
    config.progress.remaining = this.runForTime((remaining) => fun(maxIter - remaining), config.progress.maxIter, config)
    ...
    do {
      ...
      config.progress.remaining = this.runForTime((remaining) => fun(maxIter - remaining), config.progress.remaining, config)
